...
import {
  ARAMID_TOKEN_MAPPINGS,
  VOI_ARC200_TOKEN_MAPPINGS,
} from '@/constants/token-mappings';
...
function convertAramidMappingsToTokenMappings(): TokenMapping[] {
  ...
  for (const [sourceTokenId, destinationMappings] of Object.entries(
    ARAMID_TOKEN_MAPPINGS
  )) {
    for (const [destinationTokenId, mappingData] of Object.entries(
      destinationMappings
    )) {
      ...
      if (
        !tokens.some(
          (t) =>
            t.assetId === sourceToken.assetId &&
            t.networkId === sourceToken.networkId
        )
      ) {
      ...
      if (
        !tokens.some(
          (t) =>
            t.assetId === destinationToken.assetId &&
            t.networkId === destinationToken.networkId
        )
      ) {
      ...
      if ('arc200TokenId' in tokenData && tokenData.arc200TokenId) {
        ...
        if (mappingGroups.has(mappingKey)) {
          ...
          if (
            !tokens.some(
              (t) =>
                t.assetId === arc200Token.assetId &&
                t.networkId === arc200Token.networkId
            )
          ) {
  ...
  for (const [mappingKey, tokens] of mappingGroups.entries()) {
    if (tokens.length >= 2) {
      // Only include mappings with at least 2 tokens
...
export class TokenMappingService {
   ...
   */
  static getInstance(
    config?: Partial<TokenMappingConfig>
  ): TokenMappingService {
  ...
  getCachedMappings(): TokenMapping[] {
    ...
    console.log(
      `[TokenMappingService] getCachedMappings returning ${mappings.length} mappings:`,
   ...
   */
  getEquivalentTokens(assetId: number, networkId: NetworkId): TokenReference[] {
  ...
  private async fetchAndCacheMappings(): Promise<TokenMapping[]> {
    ...
    try {
      ...
      console.log(
        '[TokenMappingService] Using converted mappings as ultimate fallback'
      );
      this.cachedMappings = CONVERTED_TOKEN_MAPPINGS;
      return CONVERTED_TOKEN_MAPPINGS;
    } finally {
